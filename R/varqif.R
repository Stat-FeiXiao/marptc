...
varqif <- function(Time, Status, X, id, beta,corstr) {
  ...
  for (i in 1:K) {
    ...
    if(corstr=="independence"){
      Dfdv0= t(mu22m%*%z22)%*%MASS::ginv(sqrt(mu22m))%*%M1%*%MASS::ginv(sqrt(mu22m))%*%G2%*%(mu22m%*%z22 )
    }else{
      Dfdv0= rbind(t(mu22m%*%z22)%*%MASS::ginv(sqrt(mu22m))%*%M1%*%MASS::ginv(sqrt(mu22m))%*%G2%*%(mu22m%*%z22 )  ,
                  t(mu22m%*%z22)%*%MASS::ginv(sqrt(mu22m))%*%M2%*%MASS::ginv(sqrt(mu22m))%*%G2%*%(mu22m%*%z22 )  )
    }
