...
varrqif <- function(Time, Status, X, id, beta, gS, Lambda) {
 ...
 BBC[,1] <- rep(0,kk)
  for (j in 2:dim( xxxx)[2]) {
    for (s in 1:(kk)) {
      BCm <- gS[s] * exp(xxxx[(c1 == 1) & (t2 == tt1[s]), -1,drop = FALSE] %*% be[-1])
      BBC[s, j] <- sum(exp(xxxx[(c1 == 1) & (t2 == tt1[s]), -1,drop = FALSE] %*% be[-1]) * (exp(-BCm) + BCm * exp(-BCm) - 1)/((1 - exp(-BCm))^2) * xxxx[(c1 == 1) & (t2 == tt1[s]), j]) + sum(gg1[t2 >=
 tt1[s]] * exp(xxxx[t2 >= tt1[s], -1,drop = FALSE] %*% be[-1]) * xxxx[t2 >= tt1[s], j])
    }
  }

  CCC <- rep(0, (kk))
  for (s in 1:(kk)) {
    CCm <- gS[s] * exp(xxxx[(c1 == 1) & (t2 == tt1[s]), -1,drop = FALSE] %*% be[-1])
    CCC[s] <- sum(exp(2 * (xxxx[(c1 == 1) & (t2 == tt1[s]), -1,drop = FALSE] %*% be[-1]) - CCm)/(1 - exp(-CCm))^2)
  }

  VA1 <- matrix(0, 2*dim(xxxx)[2], dim(xxxx)[2])
  newG11<-matrix(0, 2*dim(xxxx)[2], 1)
  ...
  BC=t( VA1)%*%MASS::ginv(C)%*%rbind(BC1,BC2)
  ...
  muA <- exp(xxxx[, -1,drop = FALSE] %*%be[-1])
  ...
  for (i in 1:K) {
    ...
    for (j in 1:kk) {
      ...
      } else eqalpha[j] <- sum(mu22[Aalpha]/(1 - exp(-gS[j] * muA22[Aalpha]))) - sum(muA22[Balpha])
    }
