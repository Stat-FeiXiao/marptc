...
gqbeta <- function(Time, Status, X,stad, Ibeta, id, itermax, eps,method,corstr) {
  ...
  iter <- 0
     ...
     repeat{
       if(method=="GEE"){
         befit <- geq(Time,Status, X=stadX, beta = beta2,  id,eps,corstr)
       }else if(method=="QIF")  befit <- qeq(Time,Status, X=stadX, beta = beta2,  id,eps,corstr)
      beta1 <- befit$beta
