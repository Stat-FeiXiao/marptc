...
marptc <- function(formula, data, id, Var = TRUE,Ibeta=NULL, stad=TRUE,boots=FALSE,nboot=100, method = "GEE", corstr="independence",itermax = 100, eps = 1e-06) {
 ...
 if (is.null(id))  stop("Id variable not found!")
  ...
  if (boots) {
   ...
   print(rrn)
      ...
      mf1 <- model.frame(formula, bootdata)
      X_boot <- model.matrix(attr(mf1, "terms"), mf1)
      X_boot <- as.matrix(X_boot)
      colnames(X_boot) <- colnames(model.matrix(attr(mf1, "terms"), mf1))
   Ysurv1 <- model.extract(mf1, "response")
  if (!inherits(Ysurv1, "Surv"))
  ...
  Status1 <- Ysurv1[, 2]
      ...
      tryboot <- try(
        withTimeout(
          gqbeta(Time1, Status1, X_boot,stad=F, Ibeta,id = id_boot,itermax, eps,method,corstr)
