...
geq <- function(Time, Status, X, beta, id, eps, corstr) {
  ...
  repeat{
    ...
    G=t(D1)%*%V1%*%S1[id==1]
    G1=-t(D1)%*%V1%*%W1%*%D1#(W1%*%D1 + diag(mu[id==1])%*% DbaseF[id==1,])
    ...
    for(i in 2:K)
    {
      ...
      G11=-t(D1)%*%V1%*%W1%*%D1#(W1%*%D1 + diag(mu[id==i])%*% DbaseF[id==i,])
      G1=G1+G11
