...
marcure <- function(formula, data, id, Var = TRUE, boots=FALSE,stad=FALSE,nboot=100, struc,  esmax = 100, eps = 1e-06) {
  ...
  X <- stats::model.matrix(attr(mf, "terms"), mf)   #[, -1]去掉截距???
  X <- as.matrix(X)
...
stadX <- X
    ...
    if(stad){
      ...
      fit$beta_var <- c( (t(AA1))%*% (varfit$var_beta) %*% t(t(AA1))  ,(beta_var1[-1]) /((apply(X1[,  -1, drop = FALSE], 2, sd)        )^2)       )                 #/ (attr(stadX ,"scaled:scale") ^2)
    }  
...
print.marcure <- function(x, ...) {
  ...
  if (x$Var) {
    bt <- array(c(x$beta2), c(1+length(x$beta2), 4))
    ...
    bt <- array(c(x$beta2), c(1+length(x$beta), 1))
    rownames(bt) <-  c(x$beta_name)
