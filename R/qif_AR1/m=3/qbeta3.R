...
qbeta <- function(Status, Lambda, X, beta, id,eps) {
  ...
  G1=matrix(0,3*dim(xxxx)[2],dim(xxxx)[2])
  for (i in 1:K) {
    M1 <- diag(n[i])
    M2 <-	matrix(0,n[i],n[i])
    M2[1,2]<-1
    ...
    W1=diag(Lambda[id==i])
    D1=diag(mu[id==i])%*%diag(rep(1,n[i]))%*%(as.matrix(xxxx[id==i,]))
    V1= sqrt(diag(1/mu[id==i])) %*%M1%*% sqrt(diag(1/mu[id==i])) 
    V2= sqrt(diag(1/mu[id==i])) %*%M2%*% sqrt(diag(1/mu[id==i])) 
    V3= sqrt(diag(1/mu[id==i])) %*%M3%*% sqrt(diag(1/mu[id==i])) 
 ...
 #   Q1=t(G1)%*%matrixcalc::svd.inverse(C)%*%G
 #   Q2=t(G1)%*%matrixcalc::svd.inverse(C)%*%G1
  Q1=t(G1)%*%Cinv%*%G
  Q2=t(G1)%*%Cinv%*%G1
 ...
 #  qbeta=qbeta-matrixcalc::svd.inverse(Q2)%*%Q1
  qbeta=qbeta-Q2inv%*%Q1


  if( all(abs( qbeta-oldbeta)<eps) ) break
  }
