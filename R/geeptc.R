#============== The main function that fits the PTC model based on GEE ==============#
#' Title  Fit the marginal semiparametric promotion time cure model using GEE method
#'
#' @description Fit marginal promotion time cure (PTC) model based on the general estimation equations (GEE) method. We consider three common correlation structures in this funciton. The baseline cumulative distribution function in PTC model is appromximated by the Bernstein polynomial.
#'
...
#'
#' @return An object of class \code{geeptc} is returned. It can be examined by \code{print.geeptc()}.
#'
#' @export geeptc

geeptc <- function(formula, data, id, Var = TRUE,Ibeta=NULL, boots=FALSE,stad=FALSE,nboot=100,N,tau=NULL, corstr="independence",  itermax = 100, eps = 1e-06) {
  call <- match.call()
  data <- data
  if (is.null(id)) {
  ...
  betafit <- gbeta(Time, Status, X,stad,Ibeta,id,itermax, eps,N,tau,corstr)
  beta1 <- betafit$beta1
  beta2 <- betafit$beta2
  rho <- betafit$rho
  pphi <- betafit$pphi
...
stadX <- X
    ...
    varfit <- vargee(Time, Status, stadX,id, beta1,Fhat,tau,gam,N,corstr)
    fit$beta_var <- diag(varfit$var_beta)
    beta_var1 <- diag(varfit$var_beta)
    if(stad){
      AA1 <- c(1, -apply(X1[, -1, drop = FALSE], 2, mean)/apply(X1[, -1, drop = FALSE], 2, sd) )
      fit$beta_var <- c( (t(AA1))%*% (varfit$var_beta) %*% t(t(AA1))  ,(beta_var1[-1]) /((apply(X1[,-1, drop = FALSE], 2, sd))^2))
    }
  ...
  if (boots) {
    ...
    for (rrn in 1:Bootsample) {
      ...
      tryboot <- try(
        withTimeout(
          gbeta(Time, Status, X,stad,Ibeta,id = id_boot,itermax, eps,N,tau,corstr)
...
#' Title Print geeptc object
#'
#' @param x an object of \code{geeptc}.
#' @param ... further arguments to be added in the \code{print.geeptc} function.
#'
#' @export print.geeptc

print.geeptc <- function(x, ...) {
  if (!is.null(cl <- x$call)) {
    cat("Call:\n")
    dput(cl)
  }
  cat("\nEstimation of Marginal Promotion Time Cure Model Based on GEE:\n")
  if (x$Var) {
