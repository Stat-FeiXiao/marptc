...
POOL1 <- POOL2<- list()
Qic <- rep(1,6)  #Qic <- rep(1,7)
 for(ii in 2:7){   #for(ii in 1:7){ 
N <- ii
beta2 <-rep(0,dim(X)[2])
Lambda <- Lambda1 
  repeat{
    SK2<-0
    repeat {
      MMM <- mbeta(Status, Lambda, X=stadX, beta = beta2,  id,struc)
      beta1 <- MMM$beta
...
BBB<- baseF(Time, Status, X = stadX, beta = beta1,N)
    gSS3 <- BBB$gSS3
    alpha_esti <-  BBB$alpha_esti
    if ((any(abs(beta1 - beta) > eps))) {  # || any(abs(gSS1-gSSS1)>eps)
      beta <- beta1
      Lambda<-gSS3
    } else  break
  }
Qic[ii-1] <-BBB$qic#Qic[ii] <-BBB$qic#
POOL1[[ii-1]] <- BBB#POOL1[[ii]] <- BBB#
POOL2[[ii-1]] <- MMM #POOL2[[ii]] <- MMM#
}
BBB <- POOL1[[which.min(Qic)]]
MMM <- POOL2[[which.min(Qic)]]
 gSS3 <- BBB$gSS3
  gS <- BBB$gS
...
pphi <- MMM $pphi
 ...
 beta2<- c(beta1[1] - sum((beta1[-1] * apply(X1[, -1, drop = FALSE], 2, mean)/apply(X1[, -1, drop = FALSE], 2, sd))), beta1[-1]/apply(X1[,  -1, drop = FALSE], 2, sd)) 
  ...
  list(beta = beta1, beta2=beta2,gSS3 = gSS3,PgSS3 = gSS31, rho = rho,
